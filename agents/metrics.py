"""
Prometheus Operational Metrics Exporter for power-side-channel-dpa-cpa.
"""
import time
...
class SystemMetricsCollector:
    ...
    def export_prometheus_text(self) -> str:
        ...
        return (
            ...
            f"system_tasks_total {system=\"power-side-channel-dpa-cpa\"} {self.tasks_total}\n\n"
            f"# HELP alerts_triggered_total Total count of alerts by urgency tier\n"
            f"# TYPE alerts_triggered_total counter\n"
            f"alerts_triggered_total {system=\"power-side-channel-dpa-cpa\",urgency=\"CRITICAL_STAT\"} {self.critical_alerts_total}\n"
            f"alerts_triggered_total {system=\"power-side-channel-dpa-cpa\",urgency=\"ELEVATED_RISK\"} {self.elevated_alerts_total}\n"
            f"alerts_triggered_total {system=\"power-side-channel-dpa-cpa\",urgency=\"ROUTINE\"} {self.routine_tasks_total}\n\n"
            f"# HELP phi_outbound_blocks_total Total PHI outbound guard blocks\n"
            f"# TYPE phi_outbound_blocks_total counter\n"
            f"phi_outbound_blocks_total {system=\"power-side-channel-dpa-cpa\"} {self.phi_blocks_total}\n\n"
            f"# HELP audit_chain_blocks_total Total HMAC-SHA256 audit blocks signed\n"
            f"# TYPE audit_chain_blocks_total counter\n"
            f"audit_chain_blocks_total {system=\"power-side-channel-dpa-cpa\"} {self.audit_blocks_total}\n\n"
            ...
            f"task_processing_duration_avg_seconds {system=\"power-side-channel-dpa-cpa\"} {avg_latency:.4f}\n"
        )
