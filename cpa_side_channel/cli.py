"""
Command-Line Interface for Correlation Power Analysis (CPA) & DPA Side-Channel Attack Agent.
"""
import argparse
import csv
import json
...
from .agents import SideChannelCoordinator

coordinator = SideChannelCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="power-side-channel-dpa-cpa", description="Correlation Power Analysis (CPA) & DPA Side-Channel Attack Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  CORRELATION POWER ANALYSIS (CPA) & DPA SIDE-CHANNEL ATTACK AGENT")
        print(f"  Domain: Hardware Security | Standard: ISO/IEC 17825 Side-Channel Testing")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting Correlation Power Analysis (CPA) & DPA Side-Channel Attack Agent on http://{args.host}:{args.port}")
